...
# Build the image
echo "Building image: stephenneal/controllers:${TAG}"
docker build -t stephenneal/controllers:"${TAG}" \
...
docker logs -f controllers

while true; do
    if [[ $(docker inspect -f '{{.State.Running}}' controllers) != true ]]; then
        break
    else
        echo "'controllers' container is still running... waiting 3 secs then checking again..."
        sleep 3
    fi
done

# Confirm it exited with code 0
if [[ $(docker inspect -f '{{.State.ExitCode}}' controllers) == 0 ]]; then
    echo "Success: Tests Passed! - stephenneal/controllers:${TAG}"
else
    echo "Error: Tests Failed! - stephenneal/controllers:${TAG}"
    exit 1
fi

# Confirm the image exists
docker image inspect stephenneal/controllers:"${TAG}" > /dev/null 2>&1
